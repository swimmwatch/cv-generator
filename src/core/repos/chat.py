...
_CHAT_STATE_KEY_PREFIX = "chat_pending:"
_CHAT_STATE_TTL = 180


class ChatStateRepository(typing.Protocol):
...
class RedisChatStateRepository:
    def __init__(self, redis_client: Redis) -> None:
        self._redis_client = redis_client

    def _key(self, user_id: uuid.UUID) -> str:
        return f"{_CHAT_STATE_KEY_PREFIX}{user_id}"

    async def set_active(self, user_id: uuid.UUID) -> None:
        await self._redis_client.set(self._key(user_id), 1, ex=_CHAT_STATE_TTL)
